...
type MongoServiceConfig struct {
	...
	IdFieldName string // BSON field name used as document ID, defaults to "id"
}
...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
    ...
    if svc.ServerHost == "" {
        svc.ServerHost = enviro("CONTRACTS_API_MONGODB_HOST", "localhost")
    }

    if svc.ServerPort == 0 {
        port := enviro("CONTRACTS_API_MONGODB_PORT", "27017")
    ...
    if svc.UserName == "" {
        svc.UserName = enviro("CONTRACTS_API_MONGODB_USERNAME", "")
    }

    if svc.Password == "" {
        svc.Password = enviro("CONTRACTS_API_MONGODB_PASSWORD", "")
    }

    if svc.DbName == "" {
        svc.DbName = enviro("CONTRACTS_API_MONGODB_DATABASE", "contracts")
    }

    if svc.Collection == "" {
        svc.Collection = enviro("CONTRACTS_API_MONGODB_COLLECTION", "contracts")
    }

    if svc.Timeout == 0 {
        seconds := enviro("CONTRACTS_API_MONGODB_TIMEOUT_SECONDS", "10")
...
func (m *mongoSvc[DocType]) CreateDocument(ctx context.Context, id string, document *DocType) error {
    ...
    result := collection.FindOne(ctx, bson.D{{Key: m.IdFieldName, Value: id}})
    switch result.Err() {
...
func (m *mongoSvc[DocType]) UpdateDocument(ctx context.Context, id string, document *DocType) error {
    ...
    _, err = collection.ReplaceOne(ctx, bson.D{{Key: m.IdFieldName, Value: id}}, document)
    return err
}
...
func (m *mongoSvc[DocType]) DeleteDocument(ctx context.Context, id string) error {
    ...
    _, err = collection.DeleteOne(ctx, bson.D{{Key: m.IdFieldName, Value: id}})
    return err
}